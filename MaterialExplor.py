...
def update_graph(x_axis_name, y_axis_name, x_min, x_max, y_min, y_max):
    ...
    if y_min is not None: regression_df = regression_df[regression_df[y_axis_name] >= y_min]
    if y_max is not None: regression_df = regression_df[regression_df[y_axis_name] <= y_max]
