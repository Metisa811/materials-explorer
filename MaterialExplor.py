...
with col_a:
    x_min, x_max = st.slider(
        ...
        step=(df[x_axis].max() - df[x_axis].min()) / 100
    )
with col_b:
    y_min, y_max = st.slider(
        ...
        step=(df[y_axis].max() - df[y_axis].min()) / 100
    )

# --- فیلتر داده بر اساس رنج ---
plot_df = df[(df[x_axis] >= x_min) & (df[x_axis] <= x_max) &
             (df[y_axis] >= y_min) & (df[y_axis] <= y_max)].copy()
